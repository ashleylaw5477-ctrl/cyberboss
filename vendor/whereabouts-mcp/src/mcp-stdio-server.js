...
function runWhereaboutsMcpServer({ toolHost }) {
...
function buildToolResources(toolCatalog) {
  ...
  const resources = [];
  resources.push({
    uri: "whereabouts://tools/index",
    name: "Whereabouts Tool Index",
    description: "Overview of whereabouts MCP tools with schemas and usage notes.",
    mimeType: "text/markdown",
    text: buildToolIndexMarkdown(tools),
  });
  for (const tool of tools) {
    resources.push({
      uri: `whereabouts://tools/${tool.name}`,
...
function buildToolIndexMarkdown(tools) {
  const lines = [
    "# Whereabouts MCP Tools",
    "",
    "These are the whereabouts MCP tools.",
    "",
  ];
  for (const tool of tools) {
...
module.exports = { runWhereaboutsMcpServer };
